...
namespace recast_navigation {
	...
	class Index_Centroid
	{
	public:
		int idx;
		float centroid;

		bool operator<(const Index_Centroid& other)const
		{
	...
	public:
		...
		virtual bool loadFromFile(const string& filename, float scale_factor = 1.0f);
	...
	private:
		...
		// 为了便于查找，存放了根据重心排序的索引表.reference set for query
		std::vector<Index_Centroid> _ref4Query;
	};
}
