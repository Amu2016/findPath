...
void recast_navigation::FileNavmeshParser::parse(const string & filename, float scale_factor)
{
	ssize_t size = 0;
	...
	for (rapidjson::SizeType i = 0; i < graph.Size(); ++i)
	{
		...
		if (poly.HasMember("verts"))
		{
			...
			{
				float x = vertsObj[j]["x"].GetDouble()*scale_factor;
				float y = vertsObj[j]["y"].GetDouble()*scale_factor;
